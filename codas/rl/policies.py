...
class MlpPolicy(FeedForwardPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, norm_obs_fn, reuse=False, **_kwargs):
        self.norm_obs_fn = norm_obs_fn
        super(MlpPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                        feature_extraction="mlp", **_kwargs)

    @property
    def processed_obs(self):
        if self.norm_obs_fn is not None:
            from stable_baselines.common.running_mean_std import RunningMeanStd
            assert isinstance(self.norm_obs_fn, RunningMeanStd)
            obs = tf.clip_by_value((self.obs_ph - self.norm_obs_fn.mean) / tf.sqrt(self.norm_obs_fn.var + 1e-8), -10, 10)
            return obs
        else:
            return self.obs_ph
